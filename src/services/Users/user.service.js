...
import {userSort,userPagination, userFilters} from '../../utils/user_sort_filters.js';
...
export const findByEmail = async (email)=> {
    if(!email) throw AppErrors.badRequest('User Email is Required / البريد الالكترونى للمستخدم مطلوب');
    if(!globalRegex.emailRegex.test(email)) throw AppErrors.badRequest('Invalid Email Format / البريد الالكترونى خطأ');
...
export const findByPhone = async (phone)=> {
    if(!phone) throw AppErrors.badRequest('User Phone is Required / رقم الهاتف للمستخدم مطلوب');
    if(!globalRegex.phoneRegex.test(phone)) throw AppErrors.badRequest('Invalid Phone Format / رقم الهاتف خطأ');
...
export const update = async (id, newData) => {
    if(!id.match(/^[0-9a-fA-F]{24}$/)) 
        throw AppErrors.badRequest('Invalid User Id / Id المستخدم خطأ');

    if(newData.email && !globalRegex.emailRegex.test(newData.email)) 
        throw AppErrors.badRequest('Invalid Email Format / البريد الالكترونى خطأ');

    if(newData.phonenumber && !globalRegex.phoneRegex.test(newData.phonenumber)) 
        throw AppErrors.badRequest('Invalid Phone Format / رقم الهاتف خطأ');
    ...
    if(newData.email && newData.email !== user.email) {
        ...
        if(existUser) throw AppErrors.conflict('Email already in use / البريد الالكترونى مستخدم مسبقا');
    }
...
export const hRemoveAll = async (ids)=>{
    if(!Array.isArray(ids)|| ids.length === 0) throw AppErrors.badRequest('IDs Array is Required / مصفوفه من Ids مطلوبه');
